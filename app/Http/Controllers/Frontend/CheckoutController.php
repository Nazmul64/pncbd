...
class CheckoutController extends Controller
{
    ...
    {
        return response()->json(LocationHelper::getDistrictsAndThanas());
    }
    ...
    public function place(Request $request)
    {
        ...
        if ($isBlocked) {
            return redirect()->back()->with('error', 'আমাদের সিস্টেম আপনাকে ধরে ফেলছে দয়া করে এডমিনের সাথে কন্টাক করুন। (IP: ' . $clientIp . ')');
        }
    ...
    private function finalizeOrder(
        ...
        try {
            ...
            try {
                \App\Models\CustomerVisit::logOrderVisit($pending['phone'], $pending['customer_name']);
            } catch (\Throwable $trackEx) {
                \Illuminate\Support\Facades\Log::error('TrackReturningCustomer log order failed: ' . $trackEx->getMessage());
            }
