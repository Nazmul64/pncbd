...
class SteadfastOrderController extends Controller
{
    ...
    {
        // Force the new Packzy API URL, ignoring any old URL saved in the database
    ...
    private function buildPayload(Order $order): array
    {
        ...
        $mainAddress = trim((string) $order->address);
        if ($mainAddress !== '') {
            $addressParts[] = $mainAddress;
        }
        
        if ($order->thana) {
            $addressParts[] = trim($order->thana);
        }
        
        if ($order->district) {
            $addressParts[] = trim($order->district);
        }
        
        $fullAddress = implode(', ', $addressParts);

        // Build a clean item description from the ordered items
        $itemsDesc = '';
        ...
        $noteText = trim((string) ($order->note ?? ''));

        // If a note exists, append it to the item description to guarantee visibility in the Steadfast portal
        if ($noteText !== '') {
            if ($itemsDesc !== '') {
                $itemsDesc .= ' (Note: ' . $noteText . ')';
            } else {
                $itemsDesc = 'Note: ' . $noteText;
            }
        }
        ...
        return [
            ...
            'notes'             => $noteText,
            'order_note'        => $noteText,
            'delivery_note'     => $noteText,
            'item_description'  => substr($itemsDesc, 0, 480),
        ];
    }
