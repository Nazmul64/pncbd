...
class FraudCheckerController extends Controller
{
    public function check(Request $request)
    {
        ...
        // Check External BD Courier API if active
        $bdCourierData = null;
        $activeApi = FraudApi::where('is_active', true)->first();
        if ($activeApi && $activeApi->api_url && $activeApi->api_key) {
            try {
                $response = Http::timeout(5)->post($activeApi->api_url, [
                    'api_key' => $activeApi->api_key,
                    'phone' => $phone
                ]);
                if ($response->successful()) {
                    $bdCourierData = $response->json();
                }
            } catch (\Exception $e) {
