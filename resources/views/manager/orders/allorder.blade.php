                ...
                <tbody>
                    ...
                    <tr id="row-{{ $order->id }}">
                        ...
                        <td>
                            ...
                            <div style="font-weight:600; color: #475569; font-size: 11px;">{{ $order->phone }}</div>
                            @php
                                $cleanedPhone = preg_replace('/[^0-9]/', '', $order->phone);
                                $last11 = strlen($cleanedPhone) >= 11 ? substr($cleanedPhone, -11) : $order->phone;
                                $orderIndex = 0;
                                if ($last11) {
                                    $orderIndex = \App\Models\Order::where('phone', 'LIKE', '%' . $last11)->where('id', '<=', $order->id)->count();
                                }
                                $prevCount = $orderIndex - 1;
                            @endphp
                            @if($orderIndex > 1)
                                <div style="margin-top: 4px;">
                                    <span class="badge" style="font-size: 9px; font-weight: 700; background-color: #e0e7ff; color: #3730a3; border: 1px solid #c7d2fe; padding: 2px 5px; border-radius: 4px; display: inline-flex; align-items: center; gap: 2px;" title="Previously ordered {{ $prevCount }} times">
                                        <i class="bi bi-people-fill"></i> Old Customer
                                    </span>
                                </div>
                            @elseif($orderIndex === 1)
                                <div style="margin-top: 4px;">
                                    <span class="badge" style="font-size: 9px; font-weight: 700; background-color: #f4f6fb; color: #718096; border: 1px solid #e2e8f0; padding: 2px 5px; border-radius: 4px; display: inline-flex; align-items: center; gap: 2px;">
