...
<div class="db-page">
    ...
    <div style="margin-top: 30px; margin-bottom: 20px;">
        <h3 style="font-size: 20px; font-weight: 700; color: #0f172a; margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
            <i class="bi bi-people-fill" style="color: #3b82f6;"></i> HRM System Overview
        </h3>
    </div>
    <div class="db-stats-grid">
        <div class="db-stat-card" style="--card-color: #3b82f6; --icon-bg: #dbeafe;">
            <div class="db-stat-icon"><i class="bi bi-person-badge-fill"></i></div>
            <div class="db-stat-info">
        ...
        <div class="db-stat-card" style="--card-color: #10b981; --icon-bg: #d1fae5;">
            <div class="db-stat-icon"><i class="bi bi-calendar-check-fill"></i></div>
            <div class="db-stat-info">
                <div class="db-stat-val">{{ number_format($attendanceToday) }}</div>
                <div class="db-stat-label">Attendance Today</div>
            </div>
        </div>
        <div class="db-stat-card" style="--card-color: #f59e0b; --icon-bg: #fef3c7;">
            <div class="db-stat-icon"><i class="bi bi-calendar-minus-fill"></i></div>
            <div class="db-stat-info">
                <div class="db-stat-val">{{ number_format($activeLeaves) }}</div>
                <div class="db-stat-label">On Leave Today</div>
            </div>
        </div>
        <div class="db-stat-card" style="--card-color: #ef4444; --icon-bg: #fee2e2;">
            <div class="db-stat-icon"><i class="bi bi-cash-coin"></i></div>
            <div class="db-stat-info">
                <div class="db-stat-val">৳{{ number_format($expensesThisMonth, 0) }}</div>
                <div class="db-stat-label">Monthly Expenses</div>
            </div>
        </div>
    </div>
    @endif

    {{-- FALLBACK WELCOMING SCREEN --}}
    @if(!$hasOrders && !$hasProducts && !$hasHRM)
    <div style="background: #fff; border-radius: 16px; padding: 50px 30px; text-align: center; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.03); border: 1px solid rgba(0,0,0,0.02); margin-top: 30px;">
        <div style="width: 80px; height: 80px; background: rgba(16, 185, 129, 0.1); color: #10b981; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 36px; margin: 0 auto 20px;">
            <i class="bi bi-shield-check"></i>
        </div>
        <h2 style="font-size: 22px; font-weight: 800; color: #0f172a; margin-bottom: 10px;">Welcome to Manager Portal</h2>
        <p style="font-size: 15px; color: #64748b; max-width: 500px; margin: 0 auto 24px; line-height: 1.6;">
            Your account is active. Please use the sidebar navigation on the left to access the specific modules and systems assigned to you.
        </p>
        <div style="display: inline-flex; align-items: center; gap: 8px; background: #f1f5f9; padding: 10px 20px; border-radius: 30px; font-size: 13.5px; font-weight: 600; color: #475569;">
            <i class="bi bi-info-circle-fill" style="color: #3b82f6;"></i> Need access to more modules? Contact your Administrator.
        </div>
    </div>
    @endif
    ...
    @if($hasOrders || $hasProducts)
    <div class="db-layout" @if(!$hasOrders || !$hasProducts) style="grid-template-columns: 1fr;" @endif>
        {{-- Chart --}}
        @if($hasOrders)
        <div class="db-card">
