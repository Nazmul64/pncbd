...
<div class="usr-page">
    <div class="usr-header">
        <h2>Edit Admin / Staff Details</h2>
        <p>'{{ $user->name }}' এর ইউজার প্রোফাইল ও অ্যাক্সেস রোল পরিবর্তন করুন</p>
    </div>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" id="name" class="form-control" placeholder="e.g. Elisa Maurer" value="{{ old('name', $user->name) }}" required>
                </div>

                {{-- Email --}}
                <div class="col-md-6">
                    <label for="email" class="form-label">ইমেইল (Email Address) <span class="text-danger">*</span></label>
                    <input type="email" name="email" id="email" class="form-control" placeholder="e.g. elisa@example.com" value="{{ old('email', $user->email) }}" required>
                </div>

                {{-- Phone --}}
                <div class="col-md-6">
                    <label for="phone" class="form-label">ফোন নম্বর (Phone Number) <span class="text-danger">*</span></label>
                    <input type="text" name="phone" id="phone" class="form-control" placeholder="e.g. 01712345678" value="{{ old('phone', $user->phone) }}" required>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="status" id="status" class="form-select" required>
                        ...
                        <option value="suspended" {{ old('status', $user->status) === 'suspended' ? 'selected' : '' }}>Suspended (স্থগিত/ব্লকড)</option>
                ...
                {{-- Password --}}
                <div class="col-md-6">
                    <label for="password" class="form-label">নতুন পাসওয়ার্ড (Password)</label>
                    <input type="password" name="password" id="password" class="form-control" placeholder="••••••••">
                    <small class="text-muted"><i class="bi bi-info-circle me-1"></i> ফাঁকা রাখলে আগের পাসওয়ার্ড অপরিবর্তিত থাকবে।</small>
                </div>
                ...
                <div class="col-12 mt-5 d-flex gap-3 justify-content-end">
                    ...
                    <button type="submit" class="btn-submit">
                        <i class="bi bi-check-circle me-2"></i> তথ্য আপডেট করুন
                    </button>
                </div>
            </div>
        </form>
